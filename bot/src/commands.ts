...
export const COMMANDS: CommandSpec[] = [
  ...
  { name: 'collision', usage: '<name> <symbol>', group: 'Admin', scope: 'admin', where: 'dm',
    what: 'the count the card would print for a name and ticker' },
  { name: 'numbers', usage: '', group: 'Admin', scope: 'admin', where: 'any',
    what: 'the daily figures as a picture' },
  { name: 'launch', usage: 'watch|unwatch|status|name|set|cancel', group: 'Admin', scope: 'admin', where: 'any',
    what: 'arm the rooms: watch the deployer from each chat, with that chat\'s own delay' },
  { name: 'seat', usage: 'add|remove|tier|note|list|history', group: 'Admin', scope: 'admin', where: 'dm',
    what: 'the roster and its notes. every view that shows either refuses to answer in a group' },
