...
test('the early full card and the default card cannot contradict each other', () => {
  ...
  assert.match(full, /2 buyers in first 47s · index median 1 at this age \(n=412\)/,
    'but the count it does have is stated');
  assert.match(full, /top 5 hold 44% · 23 holders/);

  const dflt = renderDefaultCard(r, 'b');
  for (const line of ['2 buyers in first 47s · index median 1 at this age (n=412)', 'top 5 hold 44% · 23 holders']) {
